...
DISK_STATE     DiskState[MAX_NUMBER_OF_DISKS + 1];
...
void MemoryCommon(INT32 VirtualAddress, char *data_ptr, BOOL read_or_write) {
    ...
    if (page_offset > PGSIZE - 4) /* long int wraps over page */
    {
        while (page_is_valid == FALSE ) {
            invalidity = 0;
            if (VirtualPageNumber + 1 >= VIRTUAL_MEM_PAGES)
                invalidity = 6;
    ...
    case Z502DiskStatus: {
        if (MemoryMappedIODiskDevice == -1)
            *data = ERR_BAD_DEVICE_ID;
            ...
            if (DiskState[MemoryMappedIODiskDevice].DiskInUse == TRUE)
                *data = DEVICE_IN_USE;
            else
                *data = DEVICE_FREE;
        }
...
void HardwareTimer(INT32 time_to_delay) {
    ...
    if (DO_DEVICE_DEBUG) {           // Print lots of info
        ...
        if (time_to_delay < 0)
            printf("DEVICE_DEBUG: TROUBLE - want to delay for negative time!!\n");
        if (timer_state.timer_in_use == TRUE)
        ...
        if (CurrentTimerExpirationTime
            ...
            printf("DEVICE_DEBUG: TROUBLE - you are replacing the current timer ");
            printf("value of %d with a time of %d\n",
                    CurrentTimerExpirationTime,
                    CurrentSimulationTime + time_to_delay);
            printf("DEVICE_DEBUG: which is even further in the future.  This is not wise\n");
        }
        printf(
...
void Z502Idle(void) {
    ...
    if (DO_DEVICE_DEBUG) {
        printf("\nDEVICE_DEBUG: Z502Idle: Time is = %d: ", CurrentSimulationTime);
...
void Z502MakeContext(void **ReturningContextPointer, void *starting_address,
    ...
    our_ptr->program_mode = KERNEL_MODE;     // Always start process in Kernel Mode
...
void HardwareInterrupt(void) {
    ...
    while (TRUE ) {
        ...
        if (event_type >= DISK_INTERRUPT
            ...
            if (DiskState[event_type - DISK_INTERRUPT + 1].DiskInUse == FALSE) {
                printf("False interrupt - the Z502 got an interrupt from a\n");
                printf("DISK - but that disk wasn't in use.\n");
                HardwareInternalPanic(ERR_Z502_INTERNAL_BUG);
            }

/*  I don't see why the following statment is true!!  07/2014
...
*/
            ...
            memcpy(DiskState[event_type - DISK_INTERRUPT +1].Destination,    // Bugfix 07/2014
                   DiskState[event_type - DISK_INTERRUPT +1].Source, PGSIZE );
            if (DO_DEVICE_DEBUG) {
                DataPointer = (INT32 *)DiskState[event_type - DISK_INTERRUPT +1].Source;
                printf("\nDEVICE_DEBUG: HardwareInterrupt - Moving Disk data\n");
                printf("DEVICE_DEBUG:    Source Addr = %lX  ", 
                        (unsigned long)DiskState[event_type - DISK_INTERRUPT +1].Source);
                printf("Dest Addr = %lX\n", 
                        (unsigned long)DiskState[event_type - DISK_INTERRUPT +1].Destination);
                printf("DEVICE_DEBUG:    Contents = %d  ", DataPointer[0] );
                printf("%d  ", DataPointer[1] );
                printf("%d  ", DataPointer[2] );
                printf("%d\n", DataPointer[3] );
            }
            DiskState[event_type - DISK_INTERRUPT + 1].DiskInUse = FALSE;
            // printf("3. Setting %d FALSE\n", event_type );
            DiskState[event_type - DISK_INTERRUPT + 1].EventPtr = NULL;
        }
        ...
        if (DO_DEVICE_DEBUG) {
            printf( "DEVICE_DEBUG: HardwareInterrupt: Time = %d: ", CurrentSimulationTime );
            printf( "Handling event scheduled for = %d\n", time_of_event);
            printf( "DEVICE_DEBUG:     The hardware will now enter your interrupt_handler.\n");
        }
